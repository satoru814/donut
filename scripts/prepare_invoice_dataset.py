...
def args_parser():
    ...
    parser.add_argument('--save-images-dir-s3', type=str, default='s3://ai-lab-production/chen/donut/dataset/20221010-test/images')
    parser.add_argument('--save-table-dir-s3', type=str, default='s3://ai-lab-production/chen/donut/dataset/20221010-test/tables')
    parser.add_argument('--save-jsonl-dir-s3', type=str, default='s3://ai-lab-production/chen/donut/dataset/20221010-test/')
    parser.add_argument('--jsonl-filename', type=str, default='metadata.jsonl')
...
def main(args):
    ...
    def prepare_sample(row):
        ...
        dicti["file_name"] = os.path.join(args.training_data_dir, f'{str(receipt_id_sp)}.png')
        ...
        Path(os.path.join(args.save_images_dir_local, destname+'.png')).copy(os.path.join(args.save_images_dir_s3, destname+'.png'))        
        return dicti
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor :
        ...
        wr.s3.upload('./metadata.jsonl', os.path.join(args.save_jsonl_dir_s3, args.jsonl_filename))
